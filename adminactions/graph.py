...
def graph_form_factory(model):
    ...
    model_fields = [(f.name, f.verbose_name)
                    for f in model._meta.fields if not f.primary_key]
        ...
        if form.is_valid():
            ...
            try:
                ...
                if isinstance(field, ForeignKey):
                    ...
                    for value, cnt in cc:
                        data_labels.append(
                            force_unicode(dict(field.flatchoices).get(value, value), strings_only=True))
                else:
                    data_labels = [str(l) for l, v in cc]
                ...
                messages.error(
                    request, 'Unable to produce valid data: %s' % str(e))
    elif request.method == 'POST':
        # total = queryset.all().count()
        initial = {helpers.ACTION_CHECKBOX_NAME: request.POST.getlist(
