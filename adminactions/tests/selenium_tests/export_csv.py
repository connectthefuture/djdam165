...
class ExportCSVFireFox(FireFoxLiveTest):
    ...
    def _go_to_page(self):
        ...
        # select all
        driver.find_element_by_xpath("//input[@id='action-toggle']").click()
        # unselect sax
        driver.find_element_by_xpath(
            "//input[@name='_selected_action' and @value='1']").click()
        Select(driver.find_element_by_name("action")
               ).select_by_visible_text("Export as CSV")
    ...
    def _test(self, target, format, sample_num):
        ...
        sleep(1)
        sample = driver.find_elements_by_css_selector(
            "span.sample")[sample_num]
