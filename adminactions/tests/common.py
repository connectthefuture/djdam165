...
class BaseTestCaseMixin(object):
    ...
    def assert_post_form(self, response, context_forms=None):
        ...
        if response.status_code == 200:
            ...
            msg = "Access denied for user {0} to url `{1}`".format(
                self.logged_user, response.request['PATH_INFO'])
        ...
        def handler_factory(name):
            def myhandler(sender, action, request, queryset, **kwargs):
                ...
                self.assertSequenceEqual(
                    queryset.order_by('id').values_list('id', flat=True), self.selected_rows)
            return myhandler
