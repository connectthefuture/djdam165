...
class MergeTestApi(BaseTestCaseMixin, TransactionTestCase):
    ...
    def test_merge_success_related_all(self):
        master = User.objects.get(pk=self.master_pk)
        other = User.objects.get(pk=self.other_pk)
        entry = other.logentry_set.get_or_create(
            object_repr='test', action_flag=1)[0]
...
class MergeTest(BaseTestCaseMixin, TransactionTestCase):
    ...
    def test_error_if_too_many_records(self):
        response = self.client.post(
            self.url, {'action': 'merge', 'select_across': 0, '_selected_action': [2, 3, 4]})
        self.assertEqual(response.status_code, 302)
        self.assertIn(
            "Please select exactly 2 records", response.cookies['messages'].value)

    def _action_init(self):
        FIRST, SECOND = 2, 3
        url = reverse('admin:auth_user_changelist')
        base_data = {'action': 'merge', 'select_across':
                     0, '_selected_action': [FIRST, SECOND]}
        ...
        assert other.username == other_data['username'], (
            other.username, other_data['username'])
        assert master.username == master_data['username'], (
            master.username, master_data['username'])
        assert data.get('username') == master_data[
    ...
    def test_success(self):
        ...
        data['last_login'] = response.context[
            'formset'][0]['last_login'].value()
        data['date_joined'] = response.context[
            'formset'][0]['date_joined'].value()
        ...
        del data['preview']
        data['apply'] = 'apply'
        assert data['first_name'] == other_data[
