...
class DependenciesCollector(Collector):
    ...
    def collect(
        self, objs, source=None, nullable=False, collect_related=True, source_attr=None,
            reverse_dependency=False):
        super(
            DependenciesCollector, self).collect(objs, source, nullable, collect_related, source_attr,
                                                 reverse_dependency)
        alls = []
...
class FixtureOptions(forms.Form):
    ...
    serializer = forms.ChoiceField(
        choices=zip(get_serializer_formats(), get_serializer_formats()))
...
def _dump_qs(form, queryset, data, filename):
    ...
    ret = json.serialize(
        data, use_natural_keys=form.cleaned_data.get('use_natural_key', False),
        indent=form.cleaned_data.get('indent'))

    response = HttpResponse(mimetype='text/plain')
    if not form.cleaned_data.get('on_screen', False):
        filename = filename or "%s.%s" % (
            queryset.model._meta.verbose_name_plural.lower().replace(" ", "_"), fmt)
    ...
    if 'apply' in request.POST:
        ...
        if form.is_valid():
            try:
                ...
                return
            try:
                _collector = ForeignKeysCollector if form.cleaned_data.get(
                    'add_foreign_keys') else FlatCollector
                c = _collector(None)
                c.collect(queryset)
                adminaction_end.send(sender=modeladmin.model,
                                     action='export_as_fixture',
